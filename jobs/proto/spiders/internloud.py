import scrapy
import os
class InternshipCloudSpider(scrapy.Spider):
    name = "internship"
    allowed_domains = ["internshala.com"]
    custom_settings = {
        "FEEDS": {
            os.path.join("data", "CloudInternship.csv"): { 
                'format': 'csv'
            }
        }
    }
    ...
    def parse(self, response):
            ...
            for title in titles:
                    ...
                    follow_next_page = f"https://internshala.com/internships/cloud-computing-internship/page-{count}/"
                    count+=1
                    b+=1
                    yield{
                            'title': t,
