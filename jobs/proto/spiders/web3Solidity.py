...
class Web3SoliditySpider(scrapy.Spider):
    custom_settings = {
         "FEEDS" : {
                 os.path.join("data", "Web3solidity.csv"): {
                'format': 'csv'
            }
        }
    }
    ...
    def start_requests(self):
            url = "https://web3.career/solidity-jobs"
    ...
    def parse(self, response):
        ...
        for title in titles:
             ...
             job_url = f"https://web3.career/solidity-jobs{url[0]}"
             follow_next_page = f"https://web3.career/solidity-jobs/?page={count}"
             count += 1
             yield{
                  "title": tits,
                  "date": date,
