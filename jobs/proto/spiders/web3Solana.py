...
class Web3SolanaSpider(scrapy.Spider):
    custom_settings = {
         "FEEDS" : {
                 os.path.join("data", "web3solana.csv"): {
                'format': 'csv'
            }
        }
    }
    ...
    def start_requests(self):
            url = "https://web3.career/solana-jobs"
    ...
    def parse(self, response):
        ...
        for title in titles:
             ...
             job_url = f"https://web3.career/solana-jobs{url[0]}"
             follow_next_page = f"https://web3.career/solana-jobs/?page={count}"
             count += 1
             yield{
                  "title": tits,
                  "date": date,
