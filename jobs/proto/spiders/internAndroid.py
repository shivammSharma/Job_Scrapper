import scrapy
import os
class InternshipAndroidSpider(scrapy.Spider):
    name = "internshipAndroid"
    allowed_domains = ["internshala.com"]
    custom_settings = {
        "FEEDS": {
            os.path.join("data", "AndroidInternship.csv"): { 
                'format': 'csv'
            }
        }
    }
    ...
    def parse(self, response):
            ...
            for title in titles:
                    ...
                    follow_next_page = f"https://internshala.com/internships/android-app-development-internship/page-{count}/"
                    count+=1
                    b+=1
                    yield{
                            'title': t,
